...
if attack_method == 2
    %选择一个被连续攻击的执行体
%     redundancy = 3;
    Resistant_attack_executor_num = randperm(redundancy,1);
    Attack_exexcutors = randperm(5,Resistant_attack_executor_num);
    fprintf(' \n异构执行体：%7s  将受到连续攻击\n ', ExecutorNames(Attack_exexcutors));
    SC_attack_frame = round(total_frame_num * rand(1));
    BP_attack_frame = round(total_frame_num * rand(1));
    SCAN_attack_frame = round(total_frame_num * rand(1));
    SCL_attack_frame = round(total_frame_num * rand(1));
    SSC_attack_frame = round(total_frame_num * rand(1));
end
            ...
            case 2
                % DHR polar码仿真 
                 [ThreeSystemBERNum , ThreeSystemFERNum ,FourSystemBERNum , FourSystemFERNum,FiveSystemBERNum , FiveSystemFERNum, ...
                    ...
                    TotalExecutorsBER , TotalExecutorsFER , TotalNoneAttackExecutorsBERNum,TotalNoneAttackExecutorsFERNum,TotalNoneAttackExecutorsBER,...
                    TotalNoneAttackExecutorsFER,EachFrameExecutorsBERs ,EachFrameNoneAttackExecutorsBERs ]  =     ...
                    NewSingleSNRSimulationResistantAttack11111(N,redundancy,K(jj),ebn0(j),total_frame_num,max_frame_num,construction_method,Attack_exexcutors,     ...
...
%         end
        ...
        if K(jj) == 128 && ebn0(j) == 1.5
            ...
            hold on;
            title('码率为1/2，信噪比为1.5时，各编译码方法的实时误码率');
            xlabel('时长（一帧运行的时间）')
            ylabel('误码率')
        ...
        if K(jj) == 192 && ebn0(j) == 1.5
            ...
            plot(1:1:total_frame_num,EachFrameExecutorsBERs(5,:),'--','LineWidth',1.5,'Color',[0.75,0.75,0.75])
            hold on;
            title('码率为3/4，信噪比为1.5时，各编译码方法的实时误码率');
            xlabel('时长（一帧运行的时间）')
            ylabel('误码率')
            legend('BER of DHR polar decoder','BER of SC polar decoder','BER of BP polar decoder' ,...
                'BER of SCAN polar decoder','BER of SCL polar decoder','BER of SSC polar decoder')
        ...
        if K(jj) == 192 && ebn0(j) == 2
            ...
            plot(1:1:total_frame_num,EachFrameExecutorsBERs(5,:),'--','LineWidth',1.5,'Color',[0.75,0.75,0.75])
            hold on;
            title('码率为3/4，信噪比为2时，各编译码方法的实时误码率');
            xlabel('时长（一帧运行的时间）')
            ylabel('误码率')
            legend('BER of DHR polar decoder','BER of SC polar decoder','BER of BP polar decoder' ,...
                'BER of SCAN polar decoder','BER of SCL polar decoder','BER of SSC polar decoder')
