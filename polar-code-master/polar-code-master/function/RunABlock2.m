function  [BER,FER,u_ThisExecutor] = RunABlock2(N,K,u,executor,ebn0,ThisExecutorBER,ThisExecutorFER,...
...
% FER：当前执行体在该信噪比环境下执行得到的误帧数累加和，即ThisExecutorFER中的值加上这一个帧的误帧数
    ...
    F_kron_n = B;%克罗内克矩阵
        ...
        if executor==rand_executor && l >= attack_frame 
            u_attack = zeros(1,K);
            attacknums = sort(randi(K , [ceil(K * 0.7),1]));
