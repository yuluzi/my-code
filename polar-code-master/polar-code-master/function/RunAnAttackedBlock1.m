function  [BER,FER,u_ThisExecutor] = RunAnAttackedBlock1(N,K,u,attack_factor,executor,ebn0,...
    ThisExecutorBER,ThisExecutorFER,bp_iter_num,scan_iter_num,scl_list_size,decoder_tree_initial, G_set, B_set)
% 本函数用于模拟一个执行体在某一个特定误码率环境下执行一个被攻击的帧的过程，并更新本次执行后的误码数与误帧数累加和
%
% 输入：
% N:码长
% K:信息位长度
...
% FER：当前执行体在该信噪比环境下执行得到的误帧数累加和，即ThisExecutorFER中的值加上这一个帧的误帧数
    ...
    % 直接对译码后的信息随即修改一定位的比特（攻击是否合理？）
    % 生成随机错误位
    u_attack = zeros(1,K);
    attacknums = sort(randi(K , [ceil(K * attack_factor),1]));
    u_attack(attacknums) = 1;
    uhat = xor(uhat,u_attack);
